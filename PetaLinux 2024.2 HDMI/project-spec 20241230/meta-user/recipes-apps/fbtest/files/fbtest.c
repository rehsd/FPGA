	...
	for(int pos_offset = 0; pos_offset < 400; pos_offset++) {
		...
		for (int y = 100; y < 150; y++) {
			for (int x = 500; x < 600; x++) {
				...
				*((unsigned int *)(fbp + location)) = 0x0000FF; // Set pixel to red
			}
		}
		for (int y = 100; y < 150; y++) {
		...
		for (int y = 100; y < 150; y++) {
			for (int x = 700; x < 800; x++) {
				long int location = (x + vinfo.xoffset) * (vinfo.bits_per_pixel / 8) +
									(y + pos_offset + vinfo.yoffset) * finfo.line_length;
				*((unsigned int *)(fbp + location)) = 0xFF0000; // Set pixel to blue
			}
		}
		usleep(3000);
    }
...
void test3() {
    ...
    // Can create with: "sudo convert src.jpg -resize 1280x720\! -depth 8 rgb:out.rgb"
