...
void openwad(char* wadname)
{
    ...
    ssize_t bytes_read;
    bytes_read = read(wadfile, &header, sizeof header);
    ...
    bytes_read = read(wadfile, filetable, tablefilelength);
  ...
  int*		size )
{
	...
	off_t offset;
	offset = lseek(lumpinfo[i].handle, lumpinfo[i].filepos, SEEK_SET);

	ssize_t bytes_read;
	bytes_read = read(lumpinfo[i].handle, lump, lumpinfo[i].size);
