...
static const char *rehsd_snd_card_name = "rehsd-pl-snd-card";
                   ...
                   struct snd_pcm_hw_params *params)
{
    ...
    //ret = snd_soc_dai_set_pll(codec_dai, ADAU17X1_PLL, ADAU17X1_PLL_SRC_MCLK, clk_get_rate(prv->mclk), pll_rate);
    ret = snd_soc_dai_set_pll(codec_dai, 0, 0, clk_get_rate(prv->mclk), pll_rate);
    
    if (ret) {
        return ret;
    }

    //ret = snd_soc_dai_set_sysclk(codec_dai, ADAU17X1_CLK_SRC_PLL_AUTO, pll_rate, SND_SOC_CLOCK_OUT);
    ret = snd_soc_dai_set_sysclk(codec_dai, 0, pll_rate, SND_SOC_CLOCK_OUT);
    if (ret) {
        return ret;
    }
