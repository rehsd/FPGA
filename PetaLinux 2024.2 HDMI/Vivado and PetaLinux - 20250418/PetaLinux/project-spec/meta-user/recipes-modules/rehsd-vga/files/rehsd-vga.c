//adapted from Digilent's HDMI driver
...
static int rehsd_hdmi_get_modes(struct drm_connector *connector) {
    ...
    dev_info(hdmi->dev, "Using hardcoded 640x480@60 mode\n");
    ...
    mode->clock = 25175;       // Pixel clock in kHz (25.175 MHz)
    mode->hdisplay = 640;      // Horizontal resolution
    mode->hsync_start = 656;   // HSync start
    mode->hsync_end = 752;     // HSync end
    mode->htotal = 800;        // Total horizontal pixels
    mode->vdisplay = 480;      // Vertical resolution
    mode->vsync_start = 490;   // VSync start
    mode->vsync_end = 492;     // VSync end
    mode->vtotal = 525;        // Total vertical lines
    mode->type = DRM_MODE_TYPE_DRIVER | DRM_MODE_TYPE_PREFERRED;

    drm_mode_probed_add(connector, mode);
    drm_connector_update_edid_property(connector, NULL);

    return 1; // Only one hardcoded mode
}
...
enum drm_connector_status rehsd_hdmi_detect(struct drm_connector *connector,
				bool force)
	{
			...
			if (!hdmi->i2c_bus) {
				dev_info(hdmi->dev, "No I2C bus, assuming connector is connected\n");
...
static int rehsd_hdmi_create_encoder(struct rehsd_hdmi *hdmi)
{
	...
	encoder->possible_crtcs = 1; // Restrict to CRTC 2
...
#define rehsd_ENC_PREF_H 640
#define rehsd_ENC_PREF_V 480
...
MODULE_AUTHOR("rehsd");				//adapted from Digilent's HDMI driver
MODULE_DESCRIPTION("rehsd FPGA VGA driver");
