...
static int rehsd_hdmi_get_modes(struct drm_connector *connector)
{
	...
	dev_info(hdmi->dev, "rehsd-hdmi: hdmi_get_modes\n");
	...
	if (hdmi->i2c_bus) {
		...
		kfree(edid);
		dev_info(hdmi->dev, "rehsd-hdmi: edid_property updated\n");

	} else {
		dev_info(hdmi->dev, "rehsd-hdmi: hdmi_get_modes no i2c\n");
...
static int rehsd_hdmi_mode_valid(struct drm_connector *connector,
		struct drm_display_mode *mode)
{
	struct rehsd_hdmi *hdmi = connector_to_hdmi(connector);
		...
		dev_info(hdmi->dev, "rehsd-hdmi: mode OK\n");
	
		return MODE_OK;

	mode_bad:
		dev_info(hdmi->dev, "rehsd-hdmi: mode BAD\n");
		return MODE_BAD;
}
...
static
enum drm_connector_status rehsd_hdmi_detect(struct drm_connector *connector, bool force)
	{
		struct rehsd_hdmi *hdmi = connector_to_hdmi(connector);
...
{
		dev_info(hdmi->dev, "rehsd-hdmi: creaet connector\n");
		...
		dev_info(hdmi->dev, "rehsd-hdmi: about to register connector\n");
		drm_connector_register(connector);
		drm_connector_attach_encoder(connector, encoder);
		dev_info(hdmi->dev, "rehsd-hdmi: connector registered and encoder attached\n");
...
static void rehsd_hdmi_atomic_mode_set(struct drm_encoder *encoder, struct drm_crtc_state *crtc_state, struct drm_connector_state *connector_state)
{
	...
	//clk_set_rate(hdmi->clk, 74250000);  // Set clock rate to 74.25 MHz (1280x720p60)
}
...
static int rehsd_hdmi_create_encoder(struct rehsd_hdmi *hdmi)
{
	...
	dev_info(hdmi->dev, "rehsd-hdmi: create_encoder done\n");
...
static int rehsd_hdmi_bind(struct device *dev, struct device *master,
	...
	dev_info(hdmi->dev, "rehsd-hdmi: bind done\n");
	return 0;

hdmi_create_fail:
	dev_info(hdmi->dev, "rehsd-hdmi: hdmi_create_fail\n");
	drm_encoder_cleanup(&hdmi->encoder);
encoder_create_fail:
	dev_info(hdmi->dev, "rehsd-hdmi: encoder_create_fail\n");
	return ret;
}
...
static int rehsd_hdmi_parse_dt(struct rehsd_hdmi *hdmi)
{
	...
	*/
	...
	dev_info(hdmi->dev, "rehsd-hdmi: parse_dt done\n");
...
static int rehsd_hdmi_probe(struct platform_device *pdev)
{
	...
	dev_info(hdmi->dev, "rehsd-hdmi: probe start\n");
	...
	dev_info(hdmi->dev, "rehsd-hdmi: probe done\n");
